document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadData() {
        ...
        if (dataJSON) {
            ...
            if (appData.pending) { // Migración de datos viejos
    ...
    function renderGanttHead() {
        ...
        sevenDayKeys.forEach((dayKey, i) => {
            ...
            const nombreDia = diaActual.toLocaleDateString('es-ES', { weekday: 'short' });
            const fecha = diaActual.toLocaleDateString('es-ES', { day: 'numeric', month: 'numeric' });
            const esHoy = (i === 0) ? 'today' : '';
            
            headerHTML += `<th class="day-header ${esHoy}">
                <div>${nombreDia.charAt(0).toUpperCase() + nombreDia.slice(1)}</div>
                <div>${fecha}</div>
            </th>`;
        });
        headerHTML += '</tr>';
    ...
    function renderGanttBody() {
        ...
        allJobIds.forEach(jobId => {
            ...
            sevenDayKeys.forEach(dayKey => {
                ...
                if (startDateStr) {
                    const dayOffset = differenceInDays(dayKey, startDateStr); 
                    if (dayOffset >= 0) {
                        let dayCounter = 0;
                        for (const proceso of job.procesos) {
                            if (dayOffset >= dayCounter && dayOffset < (dayCounter + proceso.dias)) {
    ...
    jobForm.addEventListener('submit', function(e) {
        ...
        const procesos = processesText.split('\n')
            ...
            .map(line => {
                ...
                if (parts.length > 1) {
                    ...
                    if (!isNaN(diasNum) && diasNum > 0) { dias = diasNum; }
                }
                return { nombre: nombre, dias: dias };
            });

        if (titulo.trim() === '' || procesos.length === 0) {
            alert('Por favor, completa el título y al menos un proceso (Ej: Imprimir: 2).');
            return;
        }
    ...
    function handleDeleteJob(jobId) {
        const job = appData.jobs[jobId];
        const confirmDelete = confirm(`¿Estás seguro de que quieres eliminar la plantilla "${job.titulo}"? Esta acción no se puede deshacer.`);
...
}); // <-- ¡ESTA ES LA LLAVE QUE FALTABA! Cierra el addEventListener